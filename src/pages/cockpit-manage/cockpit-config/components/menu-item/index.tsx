...
const CockpitMenuItem: React.FC<CockpitMenuItemProps> = (props) => {
  ...
  const childElement = childrenData.map((item) => {
    return (
      <div key={uuid.v1()} className={styles.cockpitMenuChildrenItem}>
        {item.name}
      </div>
    )
  })
    ...
    <div className={styles.cockpitMenuItem}>
      <div className={styles.cockpitMenuItemTitle}>
        <div className={styles.cockpitMenuItemFoldIcon} onClick={() => toggle()}>
          {fold && <PlusSquareOutlined />}
          {!fold && <MinusSquareOutlined />}
        </div>
        <div className={styles.cockpitMenuTitleContent}>{name}</div>
        <div className={styles.cockpitMenuAddButtonSlot}>{buttonSlot}</div>
      </div>
      {!fold && <div className={styles.cockpitMenuItemContent}>{childElement}</div>}
    </div>
  )
}
