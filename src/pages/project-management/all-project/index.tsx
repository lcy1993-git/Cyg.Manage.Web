...
import CreateEngineer from './components/create-engineer';
import { TableItemCheckedInfo } from './components/engineer-table-item';
import ArrangeForm from './components/arrange-form';
...
>>>>>>> a006783058427713859dbafdd5e61af55df4ee20
  ...
  const { data, run: getGroupId, loading } = useRequest(getTreeSelectData, {
    manual: true,
  });
...
>>>>>>> a006783058427713859dbafdd5e61af55df4ee20
  ...
  const arrangeMenu = (
    <Menu>
      ...
      <Menu.Item>修改安排</Menu.Item>
      <Menu.Item>撤回安排</Menu.Item>
    </Menu>
  );

  const arrangeEvent = async () => {
    if (tableSelectData && tableSelectData.length === 0) {
      message.error('请先选择需要安排的项目！');
      return;
    }
    await getGroupId();
...
=======
  ...
  const postProjectMenu = (
    <Menu>
      <Menu.Item>申请结项</Menu.Item>
      <Menu.Item>撤回结项</Menu.Item>
      <Menu.Item>结项通过</Menu.Item>
      <Menu.Item>结项退回</Menu.Item>
    </Menu>
  );
...
>>>>>>> a006783058427713859dbafdd5e61af55df4ee20
      ...
      <Modal
        ...
        onCancel={() => setArrangeVisible(false)}
        onOk={() => submitArrangeEvent()}
        okText="提交"
        cancelText="取消"
      >
        <Form form={arrangeForm}>
          <Spin spinning={loading}>
            <ArrangeForm treeData={data} />
          </Spin>
        </Form>
      </Modal>
    </PageCommonWrap>
