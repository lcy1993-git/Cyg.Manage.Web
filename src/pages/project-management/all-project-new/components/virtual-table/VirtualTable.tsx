import React, { ComponentType, CSSProperties, Key } from 'react'
...
export type RowContext<T> = VirtualTableProps<T> &
  Pick<
    OriginParams<T>,
    'maxRowWidth' | 'rowHeight' | 'tableHeight' | 'tableWidth'
  > & {
    selectedKeys: Key[]
    updateSelectedKeysFlow: (keys: Key[], selected: boolean, rowData: T) => void
  }
...
const VirtualTable = <T extends Record<string, any>>({
  ...
  const { updateSelectedKeysFlow, selectedKeys } = useSelection<T>({
    rowSelection,
  })
