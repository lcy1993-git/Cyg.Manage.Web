    ...
    return (
      <Timeline.Item key={uuid.v1()} color="#0E7B3B">
        <ProjectProcessItem
          time={item.allotTime}
          title={showTitle}
          isArrangePerson={item.isArrange}
          users={item.users}
        />
      </Timeline.Item>
    );
    ...
    <div className={styles.projectProcessInfo}>
      <Timeline>
        <Timeline.Item color="#0E7B3B">
          <ProjectProcessItem
            time={projectInfo.createdOn}
            title={`立项(${projectInfo.createdCompanyName}-${
              projectInfo?.engineerCreatedByName ? projectInfo.engineerCreatedByName : '无'
            })`}
          />
        </Timeline.Item>
        {allotsElement}
