...
const ManualPreview: React.FC<Props> = (props) => {
  ...
  return (
    <div className={styles.pageShowFile} style={{ height: height }}>
      <div
        className={styles.pageShowFileCatalog}
        style={{ display: showDirectory ? 'block' : 'none' }}
      >
        <div className={styles.pageShowFileCatalogTitle}>
          <span>目录</span>
        </div>
        <div id="pageCatalogContainer">{elementStr}</div>
      </div>
      <div className={styles.pageShowFileContent} id="pageShowFileContent">
        <div className={styles.pageShowFileTitle}>{fileTitle}</div>
        <div id="fileContent">
