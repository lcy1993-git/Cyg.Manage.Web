...
import {
  default as markImageSrc4,
  default as markImageSrc5,
} from '@/assets/image/webgis/img_地物_房屋.png'
...
const pointStyle = function (
  ...
  fillSize = Styles[value].fillSize * scale
  strokeSize = Styles[value].strokeSize * scale
  ...
  if (isDismantle) {
    ...
    style_
      ? (pointStyles = [style, dismantleStyle, style_])
...
const line_style = function (feature: Feature, select: boolean = false) {
  ...
  if (!style) style = Styles.line['1013']
  ...
  if (style.img) {
    let text
    if (style.img.indexOf('220') > -1) text = Styles.line.selected.text1
    else text = Styles.line.selected.text2
    styles.push(
      new ClassStyle({
        text: new Text({
          placement: 'line',
...
const fzx_styles = function () {
  ...
  if (obj.lineDash) strokeOpts.lineDash = [6]
