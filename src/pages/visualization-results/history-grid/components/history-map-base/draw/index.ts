...
// 绘制完成的回调事件
export const drawEnd = (e: DrawEvent, source: VectorSource<Geometry>, closeDawType: Dispatch<SetStateAction<string>>) => {
  // 对线折线数据进行拆分
  if (e.feature.getGeometry().getType() === 'LineString') {
    setTimeout(() => {
      ...
      const features = coordinates.reduce((pre: Feature<Geometry>[], [x, y], index, origin) => {
        ...
        const lineString = new LineString([[x, y], [nextPonintX, nextPonintY]]);
        const feature = new Feature(lineString);
        feature.setStyle(featureStyle.typeTest)
    ...
    setTimeout(() => {
      ...
      newFeature.setStyle(featureStyle.type2);
      source!.removeFeature(e.feature);
      // 将拆分生成的新要素层添加至图层
