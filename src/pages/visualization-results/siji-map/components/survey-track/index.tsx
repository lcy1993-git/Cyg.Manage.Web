  ...
  return (
    <div className={styles.surveyTrackWrap}>
      <Tooltip placement="left" title={mediaSign ? '关闭多媒体标记' : '打开多媒体标记'}>
        <div
          className={classNames(styles.icon, mediaSign ? styles.active : '')}
          onClick={() => store.toggleMediaSign(!mediaSign)}
        >
      ...
      <Tooltip placement="left" title={observeTrack ? '关闭勘察轨迹' : '打开勘察轨迹'}>
        <div
          className={classNames(styles.icon, observeTrack ? styles.active : '')}
          onClick={() => store.toggleObserveTrack(!observeTrack)}
        >
          <img
            className={styles.surveyTrackImg}
            src={observeTrack ? surveyTrackLightPng : surveyTrackPng}
            alt=""
          />
        </div>
      </Tooltip>
      <Tooltip placement="left" title="定位至已选项目">
        <div className={styles.icon} onClick={onCilickPositionMap}>
          <AimOutlined />
        </div>
      </Tooltip>
    </div>
