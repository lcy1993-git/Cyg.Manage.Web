...
const simpleMixIn = (org: any, resRedData: any) => {
...
const drawCircular = (
  ...
  otherData: any[] | undefined = undefined
) => {
  if (!ctx) return
  data.forEach((item) => {
    if (!item) return
  ...
  switch (layMode) {
    case 1:
      if (arrangement && typeof arrangement === 'string') {
        const getArray = getMatrixDataDirectBurial(
          parseInt(arrangement[0]),
          parseInt(arrangement[2]),
          false
        )
    ...
    case 2:
      if (arrangement && typeof arrangement === 'string') {
        const getArray = getMatrixDataRowPipe(
          parseInt(arrangement[0]),
          parseInt(arrangement[2]),
          true
        )
