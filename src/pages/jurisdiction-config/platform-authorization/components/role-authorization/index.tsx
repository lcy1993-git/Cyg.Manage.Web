...
const RoleAuthorization: React.FC<RoleAuthorizationProps> = (props) => {
        ...
        },
        {
            ...
            render: (text: string, record: any) => {
                if (record.isAuthorized) {
                    return (
                        <span className="colorPrimary">已授权</span>
                    )
                } else {
                    return (
                        <span className="colorRed">未授权</span>
                    )
                }
            }
        },
    ...
    const tableRightSlot = (
        <>
            ...
            <Popconfirm placement="top" title={"确认进行批量移除授权吗"} onConfirm={() => batchRemoveAuthorizationEvent()} okText="确认" cancelText="取消">
