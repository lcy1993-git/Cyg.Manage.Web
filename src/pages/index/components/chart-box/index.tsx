...
const ChartBox: React.FC<ChartBoxProps> = (props) => {
  ...
  return (
    <div
      className={`${styles.chartBox} ${boxAlignClassName} ${styles[tltleWidthLevel]}`}
      ref={divRef}
    >
      <div className={styles.chartBoxLine}>
        <ChartBoxLine
          align={titleAlign}
          tltleWidthLevel={tltleWidthLevel}
          width={size.width ?? 0}
        />
      </div>
      ...
      <div className={styles.chartBoxBgAboveContent}>
        {title && <div className={styles.chartBoxTitle}>{title}</div>}

        <div className={styles.chartBoxContent}>{props.children}</div>
      </div>
    </div>
  )
}
