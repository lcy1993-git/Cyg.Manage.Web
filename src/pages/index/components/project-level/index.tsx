...
const ProjectLevel: React.FC<Props> = ({ type = 'pie', currentAreaInfo }) => {
  const { data: projectStageInfo } = useRequest(
    () =>
      getProjectLevel({ areaCode: currentAreaInfo.areaId, areaType: currentAreaInfo.areaLevel }),
    {
  ...
  const dataSum = useMemo(() => {
    if (projectStageInfo) {
      return (
        projectStageInfo?.reduce((sum, item) => {
  ...
  const getOption = (type: string, data?: any, index?: number) => {
    ...
    if (type === 'bar') {
      ...
      return barChartsOptions(projectStageInfo, 6)
    }
    return undefined
  }
