...
import { getProjectBuliding, AreaInfo, Type } from '@/services/index'
...
const ProjectClassify: React.FC<Props> = ({ type = 'pie', currentAreaInfo }) => {
  const { data: projectBuilding } = useRequest(
    () =>
      getProjectBuliding({ areaCode: currentAreaInfo.areaId, areaType: currentAreaInfo.areaLevel }),
    {
  ...
  const dataSum = useMemo(() => {
    if (projectBuilding) {
      return (
        projectBuilding?.reduce((sum, item) => {
