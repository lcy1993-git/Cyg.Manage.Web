...
const CreateMap: React.FC<CreateMapProps> = (props) => {
  ...
  const { data: areaData } = useGetSelectData(
    {
      url: '/Inventory/GetInventoryAreaList',
      extraParams: { inventoryOverviewId },
      requestSource: 'resource',
    },
    { ready: !!inventoryOverviewId, refreshDeps: [inventoryOverviewId] },
  );
  ...
  const resourceLibSearch = () => {
    return (
      <TableSearch width="208px">
        <Search placeholder="物料编号/名称" enterButton />
      </TableSearch>
    );
  };
  ...
  const inventoryTableSearch = () => {
    return (
      <div className="flex">
        <TableSearch width="208px">
          <Search placeholder="物料编号/需求公司" enterButton />
        </TableSearch>
        <TableSearch width="128px">
          <Search placeholder="" enterButton />
        </TableSearch>
      </div>
    );
  };
        ...
        <div className={styles.resourceTable}>
          {resourceData?.resourceLibId && (
            <GeneralTable
              ...
              extractParams={{ resourceLibId: resourceData?.resourceLibId }}
          ...
          <div className={styles.currentMapTable}>
            <GeneralTable
              ...
              extractParams={{
                ...
                area: '-1',
                materialId: '1329753980238946305',
              }}
              tableTitle="当前映射关系"
            />
          </div>
