...
const FeedbackDetail: React.FC<FeedbackDetailProps> = (props) => {
    ...
    const replyElement = replys.map((item: any, index: number) => {
        return (
            <ReplyComponent className={styles.replyItem} key={`reply_${index}`} time={item.createdOn ? moment(item.createdOn).format("YYYY-MM-DD hh:mm:ss") : ""} name={item.userName}>
                {item.content}
            </ReplyComponent>
        )
    })
        ...
        <div className={styles.feedbackDetail}>
            <div className={styles.feedbackDetailInfo}>
                <div className="flex">
                    ...
                    <div>
                        <span className="tipInfo">{detailInfo.createdOn ? moment(detailInfo.createdOn).format("YYYY-MM-DD hh:mm:ss") : ""}</span>
                    </div>
                </div>
            ...
            {
                replys.length > 0 &&
                <div className={styles.feedbackDetailReplyInfo}>
                    {replyElement}
                </div>
            }
