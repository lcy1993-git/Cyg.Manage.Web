...
const InfoTabs: React.FC<Props> = ({ data }) => {
  const dataSourceResult = useMemo(() => {
    ...
    if (Array.isArray(data?.materialMachineItems) && data?.materialMachineItems.length > 0) {
      const data1 = data.materialMachineItems.filter(
        (item: MaterialMachineItems) => item.type === 1
      )
      const data2 = data.materialMachineItems.filter(
        (item: MaterialMachineItems) => item.type === 2
      )
      const data3 = data.materialMachineItems.filter(
        (item: MaterialMachineItems) => item.type === 3
      )
      data1.length > 0 &&
        (materialMachineItems[0].children = data.materialMachineItems.filter(
          (item: MaterialMachineItems) => item.type === 1
        ))
      data2.length > 0 &&
        (materialMachineItems[1].children = data.materialMachineItems.filter(
          (item: MaterialMachineItems) => item.type === 2
        ))
      data3.length > 0 &&
        (materialMachineItems[2].children = data.materialMachineItems.filter(
          (item: MaterialMachineItems) => item.type === 3
        ))
    }
        ...
        <Tabs className="normalTabs noMargin">
          ...
          <TabPane tab="工作内容" key="工作内容">
            ...
            <div className={styles.workContent}>
              {JSON.stringify(data?.quotaItem?.workContent)?.replace('null', '') || ''}
            </div>
          </TabPane>
        </Tabs>
      </div>
