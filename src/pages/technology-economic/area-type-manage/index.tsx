...
const AreaTypeManage: React.FC = () => {
  ...
  const getLevelList = async (level: number, parentCode: string[], areaType?: number, firstCode?: string,) => {
  ...
  const onBlur = () => {
    ...
    if (values.includes('all')) { // 选中全部的话,清除其他选项
      form.setFieldsValue({
        secondCodes: ['all'],
        thirdCodes: ['all']
      })
      setDisabledThirdLevel(true)
    } else if (values.length > 1) { // 二级多选了,三级默认全部选中
      form.setFieldsValue({
        thirdCodes: ['all']
      })
      setDisabledThirdLevel(true)
  ...
  const thirdOnBlur = () => {
    ...
    if (values.includes('all')) { // 选择了多选,清除其余选项
      form.setFieldsValue({
        thirdCodes: ['all']
      })
    }
