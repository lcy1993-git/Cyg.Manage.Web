  ...
  (props: P & GeneralTableProps, ref: Ref<any>) => {
    ...
    const tableResultData = useMemo(() => {
      if (data) {
        ...
        let displayPageIndex = totalPage < pageIndex ? 1 : pageIndex
        return {
          items: items ?? [],
          pageIndex: displayPageIndex,
          pageSize,
          total,
          dataStartIndex: Math.floor((displayPageIndex - 1) * pageSize + 1),
          dataEndIndex: Math.floor((displayPageIndex - 1) * pageSize + (items ?? []).length),
        }
      }
      return {
        items: [],
      ...
      <div className={styles.cyGeneralTable} ref={tableRef}>
        <div className={styles.cyGeneralTableButtonContent}>
          <div className={styles.cyGeneralTableButtonLeftContent}>
            ...
            {name === '组件' ? (
              <TableSearch marginLeft="20px" label="设备分类" width="240px">
                <UrlSelect
                  allowClear
                  showSearch
