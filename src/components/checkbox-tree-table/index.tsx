...
const CheckboxTreeTable: React.FC<CheckboxTreeTableProps> = (props) => {
    ...
    const columns = [
        {
            ...
            render: (text: string, record: any) => {
                return (
                    <>
                        <span>
                            <Checkbox checked={record.hasPermission} onChange={(e) => moduleCheckEvent(e.target.checked, record.id)}>
                                {record.name}
                            </Checkbox>
                        </span>
                    </>
        ...
        },
        {
            ...
            render: (text: string, record: any) => {
                ...
                const checkboxElement = functions.map((item: any) => {
                    return (
                        <div key={item.id}>
                            <Checkbox checked={item.hasPermission} onChange={(e) => functionCheckEvent(e.target.checked, record.id, item.id)}>
                                {item.name}
                            </Checkbox>
                        </div>
                    )
    ...
    useMemo(() => {
        ...
        flattenCopyData.filter((item) => item.hasPermission).forEach((item) => {
    ...
    const tableAllCheckedButton = () => {
        return (
            <div className={styles.buttonContent}>
                <Button className="mr7" onClick={() => allCheckEvent()}>全选</Button>
