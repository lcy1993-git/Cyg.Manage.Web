...
const PhoneInfo: React.FC<PhoneInfoProps> = ({ phone, refresh, cancelPhone }) => {
  ...
  const getStepComponent = (step: Step) => {
    ...
    if (step === 0) {
      return (
        <>
          <div className={styles.minHeight60}>已经绑定手机号: {phone}</div>
      ...
      return (
        <>
          <div className={classNames(styles.minHeight60, styles.flex)}>
            ...
            <div>
              <Input style={{ width: '200px' }} placeholder="请输入您的手机号" ref={bindPhoneRef} />
            </div>
          </div>
          <div className={classNames(styles.minHeight60, styles.ml60)}>
            <Button type="primary" onClick={bindPhoneHandler}>
              下一步
            </Button>
            <Button className={styles.ml12} onClick={() => (phone ? setStep(0) : cancelPhone())}>
              取消
            </Button>
          </div>
        </>
      ...
      return (
        <>
          <div className={styles.minHeight60}>手机号: {currentPhone}</div>
          <div className={styles.minHeight60}>
            <PersonInfoModalVerificationCode
              type={1}
          ...
          <div className={classNames(styles.minHeight60, styles.flex, styles.ml60)}>
            <Button disabled={!canOkClick} type="primary" onClick={filishClickHandler}>
              绑定
            </Button>
            <Button className={styles.ml12} onClick={() => setStep(phone ? 0 : 1)}>
              取消
            </Button>
          </div>
        </>
