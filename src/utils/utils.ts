 ...
 *  @returns 平铺的数据
 * */ 
export const flatten = <P extends {}>(treeData: any, childName: string = 'children', parentId: string = "", parentIdName: string = "id"): P[] => {
 ...
 * */ 
 ...
 export const handleJurisdictionData = (needHandleData: any[]) => {
    return needHandleData.filter((item) => item.category !== 3).map(mapJurisdictionData)
 }

 const mapJurisdictionData = (data: any) => {
   const {id, name,hasPermission = true,children= []} = data;
   return {
     id,
     name,
     hasPermission,
     functions: children.filter((item: any) => item.category === 3).map((item: any) => ({name: item.name, id: item.id, hasPermission: item.hasPermission ?? true})),
