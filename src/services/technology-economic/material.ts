...
// 修改设计端物料库映射状态
export const materialMappingDesignLibraryModifyStatus

  = (Id: string) => {
  return cyRequest(() =>
    request(`${baseUrl.tecEco1}/MaterialLibrary/MaterialMappingDesignLibraryModifyStatus
`,
...
// 新增设计端物料映射
export const addMaterialMappingDesignLibrary
  = (data: SuppliesLibraryData) => {
  return cyRequest<any>(() =>
...
export const getResourceLibList = () => {
  return cyRequest<{ id: string, libName: string }[]>(() =>
    request(`${baseUrl.tecEco1}/MaterialLibrary/GetResourceLibList`,
      {method: 'GET', params: {}}),
  );
}

// 获取设计端映射物料库列表
export const getSourceMaterialMappingDesignLibraryList
  = (data: {
  "pageIndex": number,
  "pageSize": number,
  "keyWord": string
...
// 继承
export const MaterialMappingInherit = (data: {
  "inheritId": string,
  "byInheritId": string
}) => {
  return cyRequest<any>(() =>
